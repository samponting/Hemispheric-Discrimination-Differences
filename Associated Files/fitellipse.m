%
function [a,data2plot] = fitellipse(X,Y)

% FITELLIPSE  Least-squares fit of ellipse to 2D points.
%        [A,data2plot] = FITELLIPSE(X,Y) returns the parameters of the best-fit
%        ellipse to 2D points (X,Y).
%        The returned vector A contains the center, radii, and orientation
%        of the ellipse, stored as (Cx, Cy, Rx, Ry, theta_radians, area (pi* Rx * Ry)
%        data2plot is the necessary data to produce the ellipse fitted plot
%
...
%
% from: http://research.microsoft.com/en-us/um/people/awf/ellipse/
if nargin == 0
  % Create an ellipse
  t = linspace(0,2);
...
a = [uCentre, vCentre, Ru, Rv, thetarad, pi()*Ru*Rv];
...
insidepol = inpolygon(X,Y,nx,ny);

disp('Percentage of points inside the ellipse:');
disp(sum(insidepol)/size(X,1));

%white point and coordintates of the 20 hues. (white point is the last
%coordinate)
%uv = [0.194700000000000,0.493900000000000;0.185429490000000,0.492431695000000;0.177066442000000,0.488170510000000;0.170429490000000,0.481533558000000;0.166168305000000,0.473170510000000;0.164700000000000,0.463900000000000;0.166168305000000,0.454629490000000;0.170429490000000,0.446266442000000;0.177066442000000,0.439629490000000;0.185429490000000,0.435368305000000;0.194700000000000,0.433900000000000;0.203970510000000,0.435368305000000;0.212333558000000,0.439629490000000;0.218970510000000,0.446266442000000;0.223231695000000,0.454629490000000;0.224700000000000,0.463900000000000;0.223231695000000,0.473170510000000;0.218970510000000,0.481533558000000;0.212333558000000,0.488170510000000;0.203970510000000,0.492431695000000;0.194700000000000,0.493900000000000;0.194700000000000,0.463900000000000];
%figure; plot(nx,ny,'r-',X, Y, 'o',uv(:,1), uv(:,2), '*'); axis square;
%figure; p = plot(nx,ny,'r-',X, Y, 'o'); 
